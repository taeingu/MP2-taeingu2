...
public class CheckPassword {
     ...
     */
    public static boolean checkAPassword(final String password,
                                         final int minLength,
        ...
        for (int i = 0; i < passwordArr.length; i++) {
            if (CAPITAL_LETTERS.contains(passwordArr[i])) {
                capital++;
            }
            if (NUMBERS.contains(passwordArr[i])) {
                number++;
            }
            if (SPECIAL.contains(passwordArr[i])) {
                special++;
            }
        }

        if (capital < minCapitalLetter) {
            return false;
